...
void cdicdic_device::process_audio_map()
{
	...
	if (was_decoding)
	{
		...
		if (m_disc_spinup_counter >= 6 && m_audio_channel == 0)
		{
			m_data_buffer |= 0xa0;
			printf("SPECIAL!\n");
		}
...
uint16_t cdicdic_device::regs_r(offs_t offset, uint16_t mem_mask)
{
	...
	case 0x3ffe / 2:
	{
		uint16_t temp = m_data_buffer;
		m_data_buffer &= ~0x0080;
...
cdicdic_device::cdicdic_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock)
	: device_t(mconfig, CDI_CDIC, tag, owner, clock), m_intreq_callback(*this), m_memory_space(*this, ":maincpu", AS_PROGRAM), m_dmadac(*this, ":dac%u", 1U), m_scc(*this, ":maincpu"), m_cdrom(*this, ":cdrom"), m_clock2(clock)
{
}
